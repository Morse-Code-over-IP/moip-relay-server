...
The robot looks for a file called BotList.txt in the same directory the executable is in. The format of the file is one bot per line, | delimited, # in the first character position is considered a comment. The fields are
...
The robot also looks for an RSS-feed list and a periodic message file for EACH BOT LISTED IN BOTLIST.TXT, and if not found, it looks for a common set of files RaaFeeds.txt and PeriodicMessages.txt which will be used if a specific one is not present. If each bot has its own rss/msg file set thenm these common ones are not needed. . 
